...
def get_recommend_count(full_name: str) -> int:
    """Get how many times a repo has been recommended."""
    conn = get_conn()
    row = conn.execute("SELECT COUNT(*) as cnt FROM repos WHERE full_name = ?", (full_name,)).fetchone()
    conn.close()
    return row["cnt"] if row else 0


def get_all_recommend_counts() -> dict:
    """Get recommendation counts for all repos as {full_name: count}."""
    conn = get_conn()
    rows = conn.execute("SELECT full_name, COUNT(*) as cnt FROM repos GROUP BY full_name").fetchall()
    conn.close()
    return {row["full_name"]: row["cnt"] for row in rows}
