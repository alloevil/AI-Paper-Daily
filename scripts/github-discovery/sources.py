...
def fetch_search() -> list[dict]:
    ...
    date_active = (datetime.now(timezone.utc) - timedelta(days=3)).strftime("%Y-%m-%d")
    queries = [
        ...
        f"pushed:>{date_active} stars:>50 created:<2026-06-01",
    ]
...
# ── Source 4: Reddit r/opensource ──────────────────────────────────────

def fetch_reddit() -> list[dict]:
    """Fetch r/opensource hot posts and extract GitHub repo links."""
    print("[Source] Fetching Reddit r/opensource...")
    results = []

    try:
        url = "https://www.reddit.com/r/opensource/hot.json?limit=10"
        req = urllib.request.Request(url, headers={
            "User-Agent": "github-discovery-bot/1.0 (by /u/github_discovery_bot)",
    ...
    for child in children[:10]:
        ...
        # Find github.com links in url or selftext
        gh_match = re.match(r"https?://github\.com/([^/]+/[^/]+)", url)
        if not gh_match:
            gh_match = re.search(r"github\.com/([^/]+/[^/\s]+)", selftext)
        if not gh_match:
            gh_match = re.search(r"github\.com/([^/]+/[^/\s]+)", title)
        ...
        if gh_match:
            ...
            if repo:
                ...
                repo["reddit_score"] = post.get("score", 0)
                results.append(repo)
...
def fetch_all() -> list[dict]:
    ...
    # Deduplicate by full_name, keep first occurrence (prioritize trending > search > hn > reddit)
    seen = set()
    unique = []
    for r in all_repos:
