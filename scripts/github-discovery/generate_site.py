...
def card(r: dict) -> str:
    ...
    rec_count = r.get('recommend_count', 0)
    ...
    rec_html = f'<span class="rec-badge">已推荐 {rec_count} 次</span>' if rec_count > 0 else ''
...
<head>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .lang-filter {{ display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 24px; }}
    .lang-filter button {{ padding: 6px 14px; border-radius: 20px; border: 1px solid #30363d; background: #21262d; color: #e6edf3; font-size: 13px; cursor: pointer; transition: all 0.15s; }}
    ...
    .rec-badge {{ display: inline-block; padding: 1px 8px; border-radius: 10px; font-size: 11px; font-weight: 500; background: #f0883e22; color: #f0883e; border: 1px solid #f0883e44; }}
...
def main():
    ...
    # Load recommend counts from DB
    rec_counts = {}
    try:
        import sys
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        from db import get_all_recommend_counts
        rec_counts = get_all_recommend_counts()
    except Exception:
        pass
    ...
    for f in report_files:
        ...
        if m:
            ...
            # Inject recommend counts
            for r in first + repeat:
                r['recommend_count'] = rec_counts.get(r.get('name', ''), 0)
            if first or repeat:
